...
import support_file as sfc
import matplotlib.gridspec as gridspec
import matplotlib.animation as animation

#Create an object for the support 20032005.
support=sfc.SupportFilesCar()
constants = support.constants

#Load the constant values needed id the sode 16.
Ts=constants[6]
outputs=constants[10] #nomber of outputs (ast
hz = constants[11] # horizon prediction period
x_dot=constants[12] # constant longitudinal
time_length=constants[15] #duration of the conc

# Generate the refence signals
t=np.arange(0,time_length+Ts,Ts) #time from 0 to 10 seconds
r=constants[13]
f=constants[14]
psi_ref,X_ref,Y_ref=support.trajectory_generator(t,r,f)
sim_length=len(t) # number of control loop iteraions
refSignals=np.zeros(len(X_ref)*outputs)

# Build up the reference signal vector:

# refSignal = [psi_ref_0, Y_ref_0, psi_ref_1,Y_ref1....etc] ref signals are stored in this order
...
U1=0 # Input at t=-1s (steering wheel angle in rad (delta))
UTotal=np.zeros(len(t)) # To keep track all your inputs over
UTotal[0]=U1
...
for i in range(0,sim_length-1):
    ...
    x_aug_t=np.transpose([np.concatenate((states, [U1]),axis=0)])#x_aug_t = x_aug at k,x_aug_opt = x_aug predicted for next all horizon period

    #From the refSignals vector, only extract the reference values from your [current sample

    # Example: t now is 3 seconds, hz 15 samples, so from refSignals vectors, you mov
    #r=[psi_ref_3.1, Y_ref 3.1, psi ref 3.2, Y ref 3.2, psi_ref_4.5. Y_ref_4.5)
...
#Establish the limits for the real inputs (max: pi/6 radians)
    ...
    # print(i)
# print(len(Y_opt_total))
# print(len(statesTotal))
# print(len(X_ref))
print(UTotal)
# exit()
######################################### ANIMATION LOOP ########################################
...
def update_plot(num):
    ...
    car_1_front_wheel.set_data([lf*np.cos(statesTotal[num,1])-0.5*np.cos(UTotal[num]),lf*np.cos(statesTotal[num, 1])+0.5*np.cos(UTotal[num])],
                               [lf*np.sin(statesTotal[num, 1])-0.5*np.sin(UTotal[num]),lf*np.sin(statesTotal[num, 1])+0.5*np.sin(UTotal[num])])

    car_1_front_wheel_extension.set_data([lf+np.cos(statesTotal[num,1]), lf*np.cos(statesTotal[num, 1])+(0.5+40)*np.cos(UTotal[num])],
                                         [lf*np.sin(statesTotal[num,1]),lf*np.sin(statesTotal[num, 1])+(0.5+40)*np.sin(UTotal[num])])

    neutral_line_front_wheel.set_data([-50,50],
                                      [lf*np.sin(statesTotal[num,1]),lf*np.sin(statesTotal[num, 1])])
    ...
    if PID_switch!=1:
        ...
        neutral_line_front_wheel, yaw_angle_text, steer_angle, steering_wheel,\
        ...
        car_1_back_wheel, car_1_front_wheel, car_1_front_wheel_extension,\
        neutral_line_front_wheel, yaw_angle_text, steer_angle, steering_wheel,\
        yaw_angle, Y_position, car_determined

# Set up your figure properties

fig_x=16import numpy as np
import matplotlib.pyplot as plt
import support_file as sfc
import matplotlib.gridspec as gridspec
import matplotlib.animation as animation
...
copyright = ax0.text(0,20,"@ Aditya Mourya",size=15)
...
neutral_line_front_wheel,=ax1.plot([],[],'r',linewidth=1)
...
################################### End of animation ####################################
# Plot the the input delta(t) and the outputs: psift) and Y(1)

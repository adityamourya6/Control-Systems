...
class SupportFilesCar:
    ...
    def __init__(self):
        ...
        m = 1500
        Iz = 3000 #moment of inertia
        Caf = 19000 #front cornering stiffnes
        Car = 33000#rear cornering stiffnes
        lf  = 2
        lr = 3
        Ts = 0.02

        # Parameters for the lure change Jest, ver 2.8 Y_ref]
        # Higher cat reduces the overshoot
        # Matrix weights for the cost function. They must be diagonal
        Q = np.matrix('100 0;0 1') #weights for outputs (all samples,except final
        S = np.matrix('100 0;0 1') #weights for the final horizon period output
        ...
        # f=0.01*np.random.randint(2)-8.02 #frequency
        r = 4
        f = 0.01
        ...
        self.constants = [ m, Iz, Caf, Car, lf, lr, Ts, Q,S, R, outputs, hz, x_dot,r,f,time_length,PID_switch,Kp_yaw,Kd_yaw,Ki_yaw,Kp_Y,Kd_Y,Ki_Y,trajectory,lane_width]


    def trajectory_generator(self,t,r,f):
        '''This helps to generate trajectory'''

        Ts = self.constants[6]
        x_dot = self.constants[12]
        trajectory = self.constants[23]
        ...
        if trajectory == 1:
            ...
            if aaa < 0:
                ...
                bbb = -14
            y_1 = aaa*(x+self.constants[-1]-100)**2+bbb
    ...
    def state_space(self):
        ...
        #discrete the system(forward euler)
